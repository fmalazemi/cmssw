...
void MPICommunicator::connect(){
	...
	log<<"MPICommunicator::connect(). MPISource Successfully Connected. MPIController rank = "<<rank<<" and size = "<<size<<"."; 
}
...
void MPICommunicator::publish_and_listen(){
       ...
       log<<"MPICommunicator::publish_and_listen. Serivce successfully published its name "<<serviceName_<<" and waiting for MPIController to connect.";
       //listen
       MPI_Comm_accept(port_, MPI_INFO_NULL, 0, MPI_COMM_WORLD, &mainComm_);
       auto [rank, size] = rankAndSize(mainComm_); 
       log<<"MPICommunicator::publish_and_listen. MPIController is connected. MPISource rank = "<<rank<<" and size = "<<size<<".";
}

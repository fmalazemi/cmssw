...
//
MPIRecvVector::MPIRecvVector(const edm::ParameterSet& iConfig):communicatorToken_{consumes(iConfig.getParameter<edm::InputTag>("communicator"))},tagIDToken_{consumes(iConfig.getParameter<edm::InputTag>("tagID"))}, userTagID_{iConfig.getUntrackedParameter<int>("userTagID")}, outVectorData_{produces()}{
...
void MPIRecvVector::acquire(edm::Event const& iEvent, edm::EventSetup const& iSetup, edm::WaitingTaskWithArenaHolder holder){
  ...
  int tagID = iEvent.get(tagIDToken_) + userTagID_; 
	...
	std::future<void> fut = std::async(std::launch::async, &MPIRecvVector::asyncTask, this, std::move(holder), dataComm_, tagID);

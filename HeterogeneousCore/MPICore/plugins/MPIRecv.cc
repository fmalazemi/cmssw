...
//
MPIRecv::MPIRecv(const edm::ParameterSet& iConfig):communicatorToken_{consumes(iConfig.getParameter<edm::InputTag>("communicator"))},tagIDToken_{consumes(iConfig.getParameter<edm::InputTag>("tagID"))}, userTagID_{iConfig.getUntrackedParameter<int>("userTagID")}, outIntData_{produces()}, outVectorData_{produces()}{
...
void MPIRecv::asyncTask(edm::WaitingTaskWithArenaHolder holder, MPI_Comm dataComm_, int tagID) {
	...
	std::cout<<"Stream "<<sid_.value()<<" is Waiting --- TagID = "<<tagID<<"\n"; 
...
/*
	...
	MPI_Probe(0, tagID+1, dataComm_, &status);
        MPI_Get_count(&status, MPI_INT, &vectorSize);
	
        recvVectorData_.resize(vectorSize); 
	
	MPI_Recv(&recvVectorData_[0], vectorSize, MPI_INT, 0, tagID+1, dataComm_, MPI_STATUS_IGNORE);
*/
...
void MPIRecv::acquire(edm::Event const& iEvent, edm::EventSetup const& iSetup, edm::WaitingTaskWithArenaHolder holder){
  ...
  int tagID = iEvent.get(tagIDToken_) + userTagID_; 
	...
	std::future<void> fut = std::async(std::launch::async, &MPIRecv::asyncTask, this, std::move(holder), dataComm_, tagID);

...
class MPIDriver : public edm::stream::EDProducer<edm::GlobalCache<MPICommHandler>> {
public:
  explicit MPIDriver(const edm::ParameterSet&,  MPICommHandler const*);
  ~MPIDriver() override;
...
//  std::optional<MPICommHandler> communicator_;
...
//
MPIDriver::MPIDriver(const edm::ParameterSet& iConfig, MPICommHandler const* MPICommPTR):token_{produces()}, tagToken_{produces()} {
...
std::unique_ptr<MPICommHandler> MPIDriver::initializeGlobalCache(edm::ParameterSet const& iConfig) {
  ...
  (handler->comm_).connect();


  return handler;
}

void MPIDriver::globalEndJob(MPICommHandler* handler) {
...
MPICommHandler * handler = const_cast<MPICommHandler * >(globalCache());
  ...
  //iEvent.emplace(token_, &communicator_.value());
  int tagID; 
  MPI_Recv(&tagID, 1, MPI_UNSIGNED, 0, 0, (handler->comm_).Communicator(), MPI_STATUS_IGNORE);
